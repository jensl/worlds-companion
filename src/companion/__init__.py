...
def install_startup(args: argparse.Namespace, instances_path: str) -> None:
    ...
    if not appdata:
        print(
            "Error: APPDATA environment variable not found. Windows startup folder could not be determined."
        )
        sys.exit(1)
    ...
    try:
        ...
        print(
            "The companion daemon will now run automatically in the background when you log in to Windows."
        )
...
def main() -> None:
    ...
    serve_parser.add_argument(
        "--port", type=int, default=18181, help="Port to listen on"
    )
    serve_parser.add_argument(
        "--instances-dir",
        default="~/curseforge/minecraft/Instances",
        help="Path to Curseforge instances folder",
    )

    # Install sub-command
    install_parser = subparsers.add_parser(
        "install", help="Install the companion to run automatically on Windows startup"
    )
    install_parser.add_argument(
        "--host",
        default="127.0.0.1",
        help="Host interface to bind for the startup service",
    )
    install_parser.add_argument(
        "--port",
        type=int,
        default=18181,
        help="Port to listen on for the startup service",
    )
    install_parser.add_argument(
        "--instances-dir",
        default="~/curseforge/minecraft/Instances",
        help="Path to Curseforge instances folder for the startup service",
    )
